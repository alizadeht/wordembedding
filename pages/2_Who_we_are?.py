import streamlit as st
from utils import *

st.title('Who we are?')

st.header("We are:")

# Tural Alizada
st.subheader('Tural Alizada')
st.image('path_to_image_tural', caption='Tural Alizada - Lead on data management, model architecture, and data science aspects.', width=300)

# Asima Kochariyeva
st.subheader('Asima Kochariyeva')
st.image('path_to_image_asima', caption='Asima Kochariyeva - Managed project documentation and validation processes.', width=300)

# Latif Asgarov
st.subheader('Latif Asgarov')
st.image('path_to_image_latif', caption='Latif Asgarov - Focused on data collection and preprocessing.', width=300)

# Optionally, add a description for the project advisor
st.subheader('Project Advisor: Umid Suleymanov')
st.image('path_to_image_umid', caption='Umid Suleymanov - Project Advisor.', width=300)



st.write("""
         Tural Alizada - BSIT 2024
         """)
st.write("""
         
         Contact => talizada12103@ada.edu.az
         """)
st.write("""
         Asima Kochariyeva - BSIT 2024
         """)
st.write("""
        Contact => akochariyeva12137@ada.edu.az
         """)
st.write("""
         Latif Asgarov - BSIT 2024
         """)
st.write("""
        Contact => lasgarov12286@ada.edu.az
         """)

st.write("""
         Our Mentor - Umid Suleymanov
         """)
st.write("""
        Contact => usuleymanov@ada.edu.az
         """)

